...
| house syntax (single quotes, no template literals in data files) | apply PL's data-file build-breakers (whatever they are); the meta-rule is the same: a gate that catches them |
...
3. **Author the fluency bank** (the pandas/numpy ops you already have screenshotted: groupby/aggregate, merge/join, reshape/pivot, window, vectorize) — de-jargoned prompts, executed expected-DataFrames, scaffolded hints, executed debriefs.
...
6. **Author the judgment layer** on the genuinely multi-method problems (groupby.transform vs merge-back, vectorize vs apply, etc.), difficulty-gated, honesty rule enforced. Reuse the runner's `JudgmentLayer` component (re-skinned to PL's accent).
...
- **The runner already matches; the depth is the work.** PL is brand-aligned and nav-aligned — resist the urge to re-skin and instead build the content + judgment + verification systems above.

_Future note: SQL Lab's visual direction is moving toward a "judgment terminal" — navy + monospace-for-data + the red `break⌇labs` seam used as a system-wide motif (active marker, dividers, the "trap crack" on a wrong answer). When PL re-skins, build toward that shared system so the labs stay one family — but **content depth comes first, skin second.**_

---

_Questions back to PAL's SQL-Lab session go via `HQ/LEDGER.md`. Reference implementations to read directly: `product-analytics-lab/src/data/sqlLabProblems.js` (schema + judgment layer), `sqlLabDatamarts.js` (fixtures), `scripts/verify_methods.py` + `audit_sql_lab.py` + `run_sql.py` (the gates), `src/pages/SqlLabPage.jsx` (the runner + `JudgmentLayer`), `docs/SQL-CONTENT-STANDARD.md` + `docs/JUDGMENT-LAYER-SPIKE.md` (the standards)._

...
## In progress
- _(none — PyLab parked to focus on PAL. The "come-alive" program (D-PL-29) + deep linking (D-PL-30) are done; live backlog is in `NEXT.md` + `docs/PYLAB-TRACK2-BACKLOG.md`.)_

## Done (formerly Tier 1)

- ✅ **Persistent two-column solve layout (SQL Lab parity)** — shipped (0.41.0): the reveal lives in the right column, left (prompt + schema) stays fixed. Since extended with Check-vs-Submit, attempt history, and a gated reveal.
- ✅ **The come-alive program** (D-PL-29) — all 8 worlds, learning paths, difficulty ladder, placement diagnostic, 30 pandas + 30 python breadth, deep linking. See CHANGELOG 0.43.0.

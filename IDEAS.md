...
- **[Tier 1] Persistent two-column solve layout (SQL Lab parity).** Current: `pylab-solve-grid` gives two columns but only above-the-fold — model solution + debrief break out full-width below, collapsing the split. SQL Lab keeps both columns alive for the entire solve cycle: left = permanent home for title/tags/company/prompt/schema (never moves); right = editor + run result + pass/fail + model solution + debrief (all contained in the right column). Fix: move the post-submit reveal entirely into the right column. Left column stays put. This is the single biggest structural gap vs SQL Lab. (Observed 2026-06-25 by screenshot comparison.)

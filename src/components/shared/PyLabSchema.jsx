...
const cap = { fontSize: '0.68rem', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.09em', color: 'var(--text)' };
  ...
  if (inp.kind === 'df') {
    return (
      <div style={{ border: '1px solid var(--border)', borderRadius: 'var(--radius-sm)', background: 'var(--surface)', overflow: 'hidden' }}>
        ...
        {inp.rows > (inp.sample || []).length && <div style={{ ...mono, fontSize: '0.66rem', color: 'var(--text-dim)', padding: '0.25rem 0.6rem' }}>… {inp.rows - inp.sample.length} more rows</div>}
      </div>
    );
  }
...
function TargetShape({ out }) {
  ...
  return (
    <div style={{ border: '1px solid var(--border)', borderRadius: 'var(--radius-sm)', background: 'var(--surface-2)', padding: '0.55rem 0.7rem', display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
      ...
      {body}
      <span style={{ fontSize: '0.68rem', color: 'var(--text-dim)', fontStyle: 'italic' }}>values hidden on purpose — match it from your own reasoning, not an answer key.</span>
    </div>
  );
}

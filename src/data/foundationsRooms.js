...
export const FOUNDATION_ROOMS = [
    ...
    clusters: [
      ...
      },
      {
        ...
        modules: [
          ...
          { id: 'sp-metrics',    title: 'Metrics: counters, gauges, and histograms', model: 'Instrument a function with a counter, a gauge, and a histogram; hit it repeatedly and watch each metric type answer a different question about the same calls.', widget: 'live' },
          ...
          { id: 'sp-docker',     title: 'Docker: packaging the environment, for real', model: 'Write a Dockerfile for the pipeline; build and run the image; watch the \'works on my machine\' gap close because the environment now ships with the code.', widget: 'concept' },
        ],
      },
    ],
  },
    ...
    clusters: [
      ...
      },
      {
        ...
        modules: [
          ...
          { id: 'os-websocket', title: 'WebSockets: the upgraded, long-lived socket', model: 'Upgrade an HTTP connection to a WebSocket; watch the single socket stay open for both directions - one layer up from the epoll loop underneath it.', widget: 'concept' },
    ...
    clusters: [
      ...
      },
      {
        ...
        modules: [
          ...
          { id: 'wr-dns',        title: 'URLs & DNS: resolving a name to a connection', model: 'Resolve a URL into scheme/host/path/query, then resolve the host through DNS to an IP; step where the request actually goes before byte one.', widget: 'stepper' },
          { id: 'wr-rest',       title: 'REST APIs: resources, verbs, and status codes', model: 'Design a resource with verbs and status codes; call it against a toy handler; watch a verb/status mismatch turn into the wrong client behavior downstream.', widget: 'live' },
          { id: 'wr-client-server', title: 'Client & server: who initiates, who responds', model: 'Trace a request from client-initiated call to server response; swap roles and watch the same socket refuse to reverse them - the base model everything above builds on.', widget: 'concept' },
        ],
      },
      {
        ...
        modules: [
          ...
          { id: 'wr-proxy',      title: 'Reverse proxy: one address hiding many backends', model: 'Route requests through a reverse proxy to N backends; watch it terminate TLS and pick a backend in one hop - light, paired with wr-lb next door.', widget: 'concept' },
        ],
      },
    ],
  },
  ...
  {
    id: 'the-service-floor',
    track: 'branch',
    order: 14,
    title: 'The Service Floor',
    subtitle: 'What a shipped service does at runtime - identity, caches, queues, and the rate limiter.',
    accent: 'var(--accent)',
    status: 'planned',
    charterNote: 'D-PL-24 continuation. The app-layer half of the serving path - identity, caching, queues, rate limits, deploy, observe - that Wire (network bytes) and Storage (persistence engine) deliberately don\'t cover.',
    identity: 'Wire teaches the network bytes, Storage teaches the persistence engine; this room is the app-layer runtime between them - the identity check, the cache, the queue, and the rate limiter every shipped service runs into.',
    grounding: 'Designing Data-Intensive Applications (Kleppmann, caching/queue chapters) - OWASP Authentication Cheat Sheet - Redis docs (cache-aside pattern) - Kafka docs (partition/offset/ack model)',
    clusters: [
      {
        id: 'identity-and-access',
        label: 'Identity & Access',
        modules: [
          { id: 'sf-auth',      title: 'Authentication: what login actually proves', model: 'Hash and verify a password live against a stored hash; watch verification pass and fail without the plaintext ever being stored.', widget: 'live' },
          { id: 'sf-jwt',       title: 'JWT & API keys: the bearer-token model', model: 'Decode a JWT\'s header, payload, and signature live; flip one byte and watch verification fail - the bearer-token model, measured not asserted.', widget: 'live' },
          { id: 'sf-oauth',     title: 'OAuth: the delegated-auth handshake', model: 'Step the 3-legged delegated-auth flow - client, resource owner, authorization server, token - the shape only, not the full grant-type matrix.', widget: 'concept' },
        ],
      },
      {
        ...
        modules: [
          { id: 'sf-redis',     title: 'Redis caching: cache-aside, TTL, and the stampede', model: 'Drive a cache-aside read/write path with TTL expiry live; force a stampede on a hot key, then add jittered locking and watch it stop.', widget: 'live' },
          { id: 'sf-jobs',      title: 'Background jobs: worker pool, retry, and the cron model', model: 'Queue jobs into a worker pool; fail one and watch retry/backoff take it; compare against a fixed cron schedule.', widget: 'sim' },
          { id: 'sf-queues',    title: 'Message queues: partition, offset, and ack', model: 'Publish onto a partitioned queue; step consumer offset and ack/nack; watch an unacked message redeliver - ties back to cc-backpressure (room 5), the way se-nplus1 ties to wr-rtt.', widget: 'sim' },
        ],
      },
      {
        id: 'shipping-the-service',
        label: 'Shipping the Service',
        modules: [
          { id: 'sf-ratelimit', title: 'Rate limiting: token bucket and 429', model: 'Drive a token-bucket limiter live against a burst of requests; watch it 429 with Retry-After once the bucket empties, then compare a sliding window.', widget: 'live' },
          { id: 'sf-webhooks',  title: 'Webhooks: delivery, signature, and idempotency', model: 'Receive a webhook payload; verify its HMAC signature live; replay the same event and watch an idempotency key stop the duplicate write.', widget: 'live' },
        ],
      },
    ],
  },
